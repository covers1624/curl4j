...
static void xfer_info_callback(ffi_cif *, void *ret, void **args, void *user_data) {
    JNIEnv *env = getEnv();
    *((jint *) ret) = (*env)->CallIntMethod(
            env,
            user_data,
            callback,
            *(jlong *) args[0], *(jlong *) args[1], *(jlong *) args[2], *(jlong *) args[3], *(jlong *) args[4]
    );
...
JNIEXPORT jlong JNICALL Java_net_covers1624_curl4j_CurlXferInfoCallback_ffi_1callback(JNIEnv *env, jclass, jobject method) {
    ...
    return (jlong) &xfer_info_callback;
}
